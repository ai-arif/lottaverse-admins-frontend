...
export const submitSender = async (
  ...
  try {
    ...
    console.log("Commession Addresses", commession_addresses);
    console.log(
      "This is length of commession address",
      commession_addresses.length
    );

    //*****************UNCOMMENT NEEDED******************/
    const senderPercentages = await readContract(lotteryconfig, {
      abi: LOTTERY_REFERRAL_ABI,
      address: lotteryContract,
    ...
    const amountperuser =
      Number(senderPercentages) / commession_addresses.length;
    console.log(amountperuser);
    const approvalAmount = senderPercentages;
    console.log(approvalAmount);

    // const approvalAmount = 50000000;

    if (Number(balance) >= approvalAmount) {
      const { request } = await simulateContract(lotteryconfig, {
        abi: usdt,
        address: TokenContract,
        functionName: "approve",
        args: [Sender, BigInt(approvalAmount)],
      });
      const approve = await writeContract(lotteryconfig, request);
      console.log(approve);
      if (!approve) {
      ...
      const hash = await writeContract(lotteryconfig, {
        ...
        address: Sender,
        functionName: "sendTokensMultipleAddresses",
        args: [TokenContract, commession_addresses, amountperuser],
      });

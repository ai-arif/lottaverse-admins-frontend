...
import {
  submitwinner,
  thousandWinner,
} from "@/source/services/api/BlockchainServices";
...
const Index = () => {
  ...
  const submitthousandwinner = useCallback(
    ...
    //   }
    // },
    async (winner_addresses, setLoading, loading) => {
      try {
        thousandWinner(
          winner_addresses,
          setLoading,
  ...
  }, [address, fivePercent, loading]);

  const submitPremiumComission = useCallback(async () => {
    try {
